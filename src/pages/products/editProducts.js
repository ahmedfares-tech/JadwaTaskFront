...
export default function editProducts() {
    ...
    useEffect(() => {
        axiosRequest.get(`products/edit/${id}`).then(res => {
            // setProduct(res.data.product)
            setProduct(res.data.product);
            formik.setFieldValue('name', res.data.product.name);
            formik.setFieldValue('price', parseFloat(res.data.product.price));
            formik.setFieldValue('category', res.data.product.category.id);
            setPreviewImage(baseBackendURLImages + res.data.product.image);
            // console.log(res.data.product);
            // console.log(id);
            res.data.product.options.map(op => {
                setProductOptions((productOptions) => ([...productOptions, {
                    'optional': (op.optional === 1 ? true : false),
                    'name': op.option.name,
                    'value': op.value,
    ...
    const formik = useFormik({
        ...
        onSubmit: () => {
            ...
            axiosRequest.post(`products/update/${id}`, formData).then(res => {
                console.log(res);
            }).catch(err => {
                console.log(err);
            });
